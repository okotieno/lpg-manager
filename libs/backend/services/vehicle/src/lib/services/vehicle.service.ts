...
export class VehicleService extends CrudAbstractService<VehicleModel> {
  constructor(
    ...
    @InjectModel(DriverVehicleModel)
    private driverVehicleModel: typeof DriverVehicleModel
  ) {
    super(vehicleModel);
  }
  ...
  override async findAll(query: IQueryParam) {
    ...
    if (
      driverFilters &&
      (driverFilters.value || driverFilters?.values.length > 0)
    ) {
      ...
      if (drivers.length > 0) {
        restFilters.push({
          field: 'id',
          value: drivers
            .map((driverVehicle) => driverVehicle.vehicleId)
            .join(','),
          values: [],
